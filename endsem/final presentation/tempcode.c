...
const char* FINALIZER[] = {"iphone", "laptop", "computer", "desk", "mouse", "keyboard", "tablet phone"};
...
const int FINALIZER_SIZE = sizeof(FINALIZER)/sizeof(FINALIZER[0]);
char* generateSentence() {
  ...
  strcat(sentence, " ");
  strcat(sentence, FINALIZER[rand()%FINALIZER_SIZE]);
